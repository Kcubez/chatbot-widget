...
export default function RegistrationsPage({ params }: { params: Promise<{ botId: string }> }) {
  ...
  const load = async () => { setLoading(true); try { const res = await fetch(`/api/bots/${botId}/education-registrations`); const data = await readResponse(res); if (!res.ok) throw new Error(data.error || 'Could not load registrations'); setItems(data.registrations || []); } catch (e: any) { toast.error(e.message || 'Could not load registrations'); } finally { setLoading(false); } };
  useEffect(() => { load(); }, [botId]);
  const update = async (id: string, action: string) => { setSaving(id); try { const res = await fetch(`/api/bots/${botId}/education-registrations`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, action, scheduleText: schedule[id], adminNote: note[id] }) }); const data = await readResponse(res); if (!res.ok) throw new Error(data.error || 'Update failed'); toast.success(action === 'offer_schedule' ? 'Schedule sent to customer' : 'Customer notified'); await load(); } catch (e: any) { toast.error(e.message || 'Update failed'); } finally { setSaving(null); } };
