...
type Registration = { id: string; messengerSenderId: string; classType: string | null; learningMode: string | null; township: string | null; status: string; scheduleText: string | null };
...
export default function RegistrationsPage({ params }: { params: Promise<{ botId: string }> }) {
  ...
  const counts = useMemo(() => ({
    ...
    complete: items.filter(item => item.status === 'handed_to_human').length,
  }), [items]);
  const shown = items.filter(item => filter === 'all' || (filter === 'needs_review' && (item.status === 'pending_admin' || item.status === 'customer_requested_change')) || (filter === 'active' && item.status === 'schedule_offered') || (filter === 'completed' && (item.status === 'handed_to_human' || item.status === 'not_available')));
  ...
  return <div className="mx-auto max-w-5xl space-y-7 pb-12">
    ...
    <section className="grid gap-3 sm:grid-cols-3">
      {[{ label: 'Needs review', value: counts.review, icon: ClipboardCheck, tone: 'bg-amber-50 text-amber-700' }, { label: 'Awaiting customer', value: counts.active, icon: CalendarClock, tone: 'bg-blue-50 text-blue-700' }, { label: 'Handed to admin', value: counts.complete, icon: CheckCircle2, tone: 'bg-emerald-50 text-emerald-700' }].map(stat => <Card key={stat.label} className="border-zinc-100 shadow-sm"><CardContent className="flex items-center gap-4 p-5"><div className={`flex h-11 w-11 items-center justify-center rounded-xl ${stat.tone}`}><stat.icon className="h-5 w-5" /></div><div><p className="text-2xl font-black text-zinc-900">{stat.value}</p><p className="text-xs font-medium text-zinc-500">{stat.label}</p></div></CardContent></Card>)}
    </section>
